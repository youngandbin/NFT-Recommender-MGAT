...
class MMMO(torch.nn.Module):
    ...
    def forward(self, user_nodes, pos_items, neg_items):
        ...
        tr_rep = self.tr_gnn(self.id_embedding) # torch.Size([num_user+num_item, 512])
        self.v_representation = v_rep
        self.t_representation = t_rep
        self.p_representation = p_rep
        self.tr_representation = tr_rep

        representation = (v_rep + t_rep + p_rep + tr_rep) / 4 # torch.Size([num_user+num_item, 512])
        ...
        # calculate attention
        attention = torch.matmul(Q, K.transpose(1, 0)) / np.sqrt(K.shape[1]) # torch.Size([2048, 4])
        attention = F.softmax(attention, dim=1)
        attention = torch.matmul(attention, V) # torch.Size([2048, 512])
