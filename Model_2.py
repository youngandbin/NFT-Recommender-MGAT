...
class MMMO_2(torch.nn.Module):
    def __init__(self, features, user_features, edge_index, batch_size, num_user, num_item, reg_parm, dim_x, DROPOUT, path=None, cluster_dict=None):
        ...
        # linear layer
        self.MLP_price = MLP_price(dim_in=dim_x*4, dim_out=1)

        # cluster dict
        self.cluster_dict = cluster_dict

        # linear layer (for attention)
        self.q_fc = nn.Linear(dim_x*2, dim_x*2, bias=False) # (d_model, d_model)
        self.k_fc = nn.Linear(dim_x*2, dim_x*2, bias=False) # (d_model, d_model)
        self.v_fc = nn.Linear(dim_x*2, dim_x*2, bias=False) # (d_model, d_model)
